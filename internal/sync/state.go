...
type Snapshot struct {
	...
	// Binary is a compatibility DTO for older pages. The source client is
	// always installed, so it never represents a downloadable executable.
...
// probeTTL is how long the cached account probe is reused. It makes a network
// call, so it must not run on the page's poll interval.
...
func (e *Engine) State(ctx context.Context) Snapshot {
	...
	snapshot := Snapshot{
		...
		Account:  account,
		Binary:   aliyunpan.BuiltInBinaryState(),
		Login:    e.loginState(),
		Jobs:     current.Jobs,
		Paused:   paused,
		Now:      now.UnixMilli(),
...
// probe returns the cached account, refreshing it in the background when it
// has gone stale.
func (e *Engine) probe(ctx context.Context) AccountView {
	e.probeMu.Lock()
	cache := e.probes
...
// RefreshProbe re-reads the account state now.
func (e *Engine) RefreshProbe(_ context.Context) AccountView {
	e.invalidateProbe()
	e.requestProbeRefresh()
	e.probeMu.Lock()
	defer e.probeMu.Unlock()
	return e.probes.account
}
